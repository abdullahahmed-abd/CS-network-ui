...
function TransferTokensModal({ user, myTransferablePoints = 0, isGlobalAdmin = false, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isOverBalance) {
      setError(`Cannot transfer ${numAmount} points. You only have ${myTransferablePoints} transferable points available.`);
      return;
    }
    ...
    try {
      const res = await transferTrustTokens({
        targetUserId: String(targetUserId),
        amount: String(amount),
      });

      setSuccessMsg(res?.message || `Successfully transferred ${amount} Trust Tokens to ${targetUserName}!`);
      if (onSuccess) onSuccess();
      setTimeout(() => {
        onClose();
      }, 1800);
    } catch (err) {
      console.error('Failed to transfer trust tokens:', err);
      setError(err?.message || 'Failed to transfer trust tokens. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div style={{
            padding: '20px 24px', borderBottom: '1px solid #E2E8F0',
            background: 'linear-gradient(135deg, #0D9488 0%, #0F766E 100%)', color: '#fff',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            borderTopLeftRadius: 24, borderTopRightRadius: 24, flexShrink: 0,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.85)', margin: '2px 0 0' }}>
                  Direct P2P Reputation Token Transfer
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{
                  width: 48, height: 48, borderRadius: '50%', background: '#0F766E',
                  color: '#fff', display: 'flex', alignItems: 'center', justify: 'center',
                  fontWeight: 800, fontSize: 18, border: '2px solid #99F6E4',
                }}>
                  {targetUserName[0]?.toUpperCase()}
                </div>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 16, fontWeight: 800, color: '#0F172A' }}>{targetUserName}</span>
                    <span style={{
                      background: '#CCFBF1', color: '#0F766E', fontSize: 11, fontWeight: 800,
                      padding: '2px 8px', borderRadius: 12, border: '1px solid #5EEAD4',
                    }}>
                      User ID: #{targetUserId}
                    </span>
                  </div>
            ...
            {/* Transferable Points Balance Banner */}
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: '#0F766E' }}>
                Your Available Transferable Points
              </div>
              <div style={{ fontSize: 18, fontWeight: 900, color: '#0D9488' }}>
                {myTransferablePoints} <span style={{ fontSize: 12, fontWeight: 700 }}>pts</span>
              </div>
            </div>
            ...
            <div>
              <label style={{ fontSize: 13, fontWeight: 700, color: '#334155', marginBottom: 8, display: 'block' }}>
                Token Amount to Transfer <span style={{ color: '#DC2626' }}>*</span>
              </label>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <input
                  type="number"
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: myTransferablePoints <= 0 ? 'not-allowed' : 'pointer',
                    }}
                  >
                    MAX
                  </button>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
              ...
              >
                {submitting ? 'Transferring...' : '💸 Transfer Points'}
              </button>
            </div>
          </form>
        </motion.div>
...
function TransferTokensSubTab({ myTransferablePoints = 0, onTransferSuccess, isGlobalAdmin = false }) {
  ...
  const loadDirectoryData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await searchDirectory(filters);
      setResponseData(res);
    } catch (err) {
      console.error('Failed to load member directory for token transfer:', err);
      setError(err?.message || 'Failed to fetch member directory.');
    } finally {
      setLoading(false);
    }
  }, [filters]);
  ...
  useEffect(() => {
    loadDirectoryData();
  }, [filters.page, loadDirectoryData]);
  ...
  const membersPage = responseData?.members || { content: [], page: 0, totalPages: 1, totalElements: 0 };
  const membersList = membersPage.content || [];
  const totalElements = membersPage.totalElements || membersList.length;
  const totalPages = membersPage.totalPages || Math.ceil(totalElements / filters.size) || 1;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        borderRadius: 20, padding: '24px', color: '#fff',
        display: 'flex', flexDirection: 'column', gap: 16, boxShadow: '0 10px 25px -5px rgba(15,118,110,0.3)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 24 }}>💸</span>
              <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0, color: '#fff' }}>
                Transfer Trust Tokens to Members
              </h2>
            </div>
            <p style={{ fontSize: 13, color: '#CCFBF1', margin: '4px 0 0' }}>
              Search member directory, inspect profiles, and transfer your available trust tokens directly.
            </p>
          </div>

          <div style={{
            background: 'rgba(255,255,255,0.15)', padding: '10px 18px', borderRadius: 14,
            border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', gap: 10,
          }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#E6FFFA' }}>Transferable Balance:</span>
            <span style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>{myTransferablePoints} pts</span>
          </div>
        </div>

        {/* Filter Bar */}
        <form onSubmit={handleSearchSubmit} style={{
          background: 'rgba(255,255,255,0.08)', borderRadius: 16, padding: '16px',
          border: '1px solid rgba(255,255,255,0.15)', display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center',
        }}>
          {/* Search Input */}
          <div style={{ flex: '1 1 240px', minWidth: 200 }}>
            <input
              type="text"
              placeholder="Search member by name, email, company..."
              value={filters.search}
              onChange={(e) => setFilters(f => ({ ...f, search: e.target.value }))}
              style={{
                width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.2)',
          ...
          <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
            ...
            <button
              ...
              style={{
                padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.2)',
                background: 'transparent', color: '#CCFBF1', fontWeight: 600, fontSize: 13,
                cursor: 'pointer',
              }}
            >
              Reset
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: 40 }}>🔍</div>
          <div style={{ fontSize: 16, fontWeight: 800, color: '#1E293B' }}>No Members Found</div>
          <div style={{ fontSize: 13, color: '#64748B', maxWidth: 400 }}>
            No members matched your search criteria. Try adjusting your search query or reset filters.
          </div>
          <button
            onClick={handleResetFilters}
            style={{
          ...
          {membersList.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{
                    width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #0D9488 0%, #0F766E 100%)',
                    color: '#fff', display: 'flex', alignItems: 'center', justify: 'center',
                    fontWeight: 800, fontSize: 18, flexShrink: 0, border: '2px solid #CCFBF1',
                  }}>
                    {name[0]?.toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {franchise && (
                      <div style={{
                        marginTop: 6, display: 'inline-block', background: '#F0FDFA', color: '#0F766E',
                        border: '1px solid #99F6E4', padding: '2px 8px', borderRadius: 8, fontSize: 11, fontWeight: 700,
                      }}>
                        🏢 {franchise}
                      </div>
                    )}
                ...
                <div style={{ display: 'flex', gap: 8, paddingTop: 4 }}>
                  ...
                  <button
                    type="button"
                    onClick={() => setSelectedUserForTransfer(item)}
                    style={{
                      flex: 1.2, padding: '8px 12px', borderRadius: 10, border: 'none',
      ...
      {!loading && membersList.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: '#0D9488', padding: '0 6px' }}>
              {filters.page + 1}
            </span>
...
export default function TrustLeaderboardTab({ isAdmin = true, userRole = 'GLOBAL_ADMIN' }) {
  ...
  const isMasterOperator = userRole === 'MASTER_OPERATOR' || userRole === 'OPERATOR' || userRole === 'GENERAL_OPERATOR' || userRole === 'SECTOR_OPERATOR';
  const isMember = userRole === 'BUYER' || userRole === 'SELLER' || userRole === 'BUSINESS_PARTNER' || userRole === 'MEMBER';

  const [activeSubTab, setActiveSubTab] = useState(
    isMasterOperator ? 'global' : isMember ? 'global' : 'leaderboard'
  );
  ...
  const loadMyProfile = useCallback(async () => {
    ...
    try {
      const res = await fetchMyTrustProfile(1, 50);
      if (res?.profileDetail) {
        setMyProfileData(res.profileDetail);
      }
    } catch (err) {
      console.warn('Failed to load my trust profile stats:', err);
    } finally {
  ...
  }, [isGlobalAdmin, loadMyProfile]);

  const summary = myProfileData?.summary || {};
  const totalScore = summary.totalScore ?? 0;
  const transferablePoints = summary.transferablePoints ?? 0;
  ...
  useEffect(() => {
    if (activeSubTab !== 'award_points' && activeSubTab !== 'transfer_tokens') {
      ...
      <div style={{
        ...
        {isMasterOperator ? (
          <>
            ...
            <button
              type="button"
              onClick={() => setActiveSubTab('transfer_tokens')}
              style={{
                display: 'flex', alignItems: 'center', gap: 8,
                padding: '10px 22px', borderRadius: 12, border: 'none',
                background: activeSubTab === 'transfer_tokens' ? '#166534' : 'transparent',
                color: activeSubTab === 'transfer_tokens' ? '#FFFFFF' : '#64748B',
                fontWeight: 800, fontSize: 14, cursor: 'pointer',
                boxShadow: activeSubTab === 'transfer_tokens' ? '0 4px 12px rgba(22, 101, 52, 0.25)' : 'none',
                transition: 'all 0.2s ease',
              }}
            >
              <span>💸</span> Transfer Tokens
            </button>
          </>
        ) : isMember ? (
          <>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              {/* Top Right Own Trust Profile Header Banner (Hidden for Global Admin) */}
              {!isGlobalAdmin && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#64748B', textTransform: 'uppercase' }}>Total Points</div>
                      <div style={{ fontSize: 15, fontWeight: 900, color: '#15803D' }}>{totalScore} pts</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 16 }}>💸</span>
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#64748B', textTransform: 'uppercase' }}>Transferable</div>
                      <div style={{ fontSize: 15, fontWeight: 900, color: '#0D9488' }}>{transferablePoints} pts</div>
                    </div>
                  </div>
                  ...
                  {ownUserId && (
                    <button
                      ...
                      style={{
                        marginLeft: 4, padding: '7px 14px', borderRadius: 10, border: 'none',
                        background: 'linear-gradient(135deg, #15803D 0%, #166534 100%)',
          ...
          {/* ── Own Trust Profile Detail Modal ── */}
          {showMyProfileModal && ownUserId && (
            <TrustProfileDetailModal
              userId={ownUserId}
