...
export default function UserFormScreen({ phone, countryCode, onComplete, onSessionExpired }) {
  ...
  const toggleRole = (role) => {
    setForm((prev) => {
      ...
      if (role === 'BUSINESS_PARTNER') {
        ...
        roles = exists ? prev.roles.filter((r) => r !== role) : ['BUSINESS_PARTNER'];
      } else {
        const exists = prev.roles.includes(role);
        roles = exists
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // ✅ roles pass karo taake App.jsx decide kare kaunsa screen
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden">
      <style>{`
        ...
        select option { background-color: white; color: #1f2937; }
        select option:checked { background: linear-gradient(to bottom, #dcfce7, #bbf7d0); color: #15803d; }
      `}</style>
          ...
          >
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="rounded-full bg-gradient-to-r from-green-100 to-emerald-100 px-3 py-1 text-xs font-bold text-green-700 shadow-sm">✓ Active</span>
                  </div>
                ...
                <div className="rounded-2xl border border-green-200 bg-green-100/60 px-5 py-3 shadow-md">
                  <p className="text-sm font-bold text-gray-900">{countryCode} {phone}</p>
                </div>
              </div>
            </motion.div>
            ...
            <div className="space-y-7">
              ...
              >
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <InputField label="Company Name" placeholder="Enter your company name"
                      value={form.companyName} error={errors.companyName}
                      onChange={(e) => updateField('companyName', e.target.value)} />
                  </div>
                  <SelectField label="Position" value={form.position}
                    onChange={(e) => updateField('position', e.target.value)} options={POSITION_OPTIONS} />
                  <SelectField label="Business Age" value={form.businessAge}
              ...
              >
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <InputField label="Alternate Phone" type="tel" placeholder="e.g. +911234567890"
                      value={form.alternatePhoneNumber} error={errors.alternatePhoneNumber}
                      onChange={(e) => updateField('alternatePhoneNumber', e.target.value)} />
                  </div>
                  <SelectField label="Contact Method" value={form.preferredContactMethod}
                    onChange={(e) => updateField('preferredContactMethod', e.target.value)} options={CONTACT_METHOD_OPTIONS} />
                  <SelectField label="Language Preference" value={form.languagePreference}
                    onChange={(e) => updateField('languagePreference', e.target.value)} options={LANGUAGE_OPTIONS} />
                </div>
              </motion.div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <InputField label="Country" placeholder="Enter your country"
                      value={form.country} error={errors.country}
            ...
            {submitError && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mt-6 flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 px-5 py-4"
              >
            ...
            >
              <button type="submit" disabled={loading}
                ...
                <span className="relative z-10 flex items-center justify-center gap-3">
                  {loading ? (
                    ...
                    <>
                      ...
                      <motion.svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}
                ...
                <motion.div className="absolute inset-0 bg-gradient-to-r from-green-700 via-emerald-700 to-green-700"
...
function InputField({ label, error, ...props }) {
  return (
    <div>
      <label className="mb-2.5 block text-sm font-bold text-gray-700">{label}</label>
      <input {...props}
