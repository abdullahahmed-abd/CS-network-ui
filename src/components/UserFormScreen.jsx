  ...
  return (
    <div ref={wrapperRef} className="relative">
      <button
        ...
        className={`flex h-full items-center gap-1.5 rounded-l-2xl border-2 border-r-0 bg-white/80 px-3 py-4 text-sm font-bold transition-all duration-200 ${open ? 'border-green-400 bg-white ring-4 ring-green-100' : 'border-gray-200/60 hover:border-green-300'
          }`}
      >
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 260 }}>
              {filtered.map((c) => (
                <button key={c.code} type="button"
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-all hover:bg-gray-50 ${value === c.code ? 'bg-green-50 text-green-700' : 'text-gray-700'
                    }`}
                >
...
function PhoneInputWithCode({ value, countryCode, onPhoneChange, onCodeChange, error }) {
  ...
  return (
    <div>
      ...
      <div className={`flex overflow-hidden rounded-2xl shadow-lg transition-all duration-300 ${error ? 'ring-4 ring-red-100' : focused ? 'ring-4 ring-green-100' : ''
        }`}>
        ...
        <input
          ...
          className={`flex-1 rounded-r-2xl border-2 border-l-0 bg-white/80 px-4 py-4 text-sm font-semibold text-gray-900 outline-none transition-all duration-300 placeholder:font-medium placeholder:text-gray-400 ${error ? 'border-red-300'
              : focused ? 'border-green-400 bg-white'
  ...
  return (
    <div ref={wrapperRef}>
      ...
      <motion.button
        ...
        className={`relative flex w-full items-center justify-between rounded-2xl px-5 py-4 text-left text-sm font-semibold shadow-lg backdrop-blur-xl transition-all duration-300 ${error ? 'border-2 border-red-300 bg-white/90 ring-4 ring-red-100'
              ...
              >
                <div className="border-b border-gray-100 bg-gradient-to-r from-gray-50 to-purple-50/40 p-3">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                    <input
                      type="text" placeholder="Search sector..." value={search}
                      ...
                      {filtered.map((opt) => {
                        ...
                        return (
                          <motion.button
                            ...
                            className={`group relative flex flex-col items-center gap-2 rounded-xl p-3 text-center transition-all duration-200 ${sel ? 'bg-gradient-to-br from-green-50 to-emerald-50 ring-2 ring-green-400'
...
function SearchableDropdown({
  ...
  return (
    <div ref={wrapperRef}>
      ...
      <motion.button
        ref={buttonRef} type="button" disabled={disabled} onClick={handleOpen}
        whileHover={{ scale: disabled ? 1 : 1.01 }} whileTap={{ scale: disabled ? 1 : 0.99 }}
        className={`relative flex w-full items-center justify-between rounded-2xl px-5 py-4 text-left text-sm font-semibold shadow-lg backdrop-blur-xl transition-all duration-300 ${disabled ? 'cursor-not-allowed border-2 border-gray-200 bg-white/40 text-gray-400'
                ...
                {options.length > 4 && (
                  <div className="border-b border-gray-100 bg-gradient-to-r from-gray-50 to-green-50/30 p-3">
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                      <input
                        type="text" placeholder="Type to search..." value={search}
                        onChange={(e) => setSearch(e.target.value)} autoFocus
                        className="w-full rounded-xl border-2 border-gray-200 bg-white py-2.5 pl-10 pr-9 text-sm font-semibold outline-none focus:border-green-400"
                      />
                      {search && (
                ...
                <div className="overflow-y-auto" style={{ maxHeight: 260 }}>
                  {externalLoading ? (
                    ...
                    filtered.map((opt, i) => {
                      ...
                      return (
                        <button
                          key={`${optVal}-${i}`} type="button"
                          onClick={() => { onSelect(opt); setOpen(false); setSearch(''); }}
                          className={`flex w-full items-center gap-3 px-5 py-3.5 text-left text-sm font-semibold transition-all ${sel ? 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700'
    ...
    <div className="rounded-2xl border-2 border-gray-100/80 bg-gradient-to-br from-gray-50/60 to-green-50/30 p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <SearchableDropdown
          ...
          placeholder={form.country ? 'Select state (optional)' : '—'}
        ...
        <SearchableDropdown
          label="City"
          placeholder={form.state ? 'Select city (optional)' : '—'}
        ...
        >
          ...
          {form.state && (
            <><ChevronDown className="h-3 w-3 -rotate-90 text-gray-400" />
              <span className="rounded-lg bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-700">{form.state}</span></>
          )}
          {form.city && (
            <><ChevronDown className="h-3 w-3 -rotate-90 text-gray-400" />
              <span className="rounded-lg bg-purple-100 px-2 py-0.5 text-xs font-bold text-purple-700">{form.city}</span></>
          )}
        </motion.div>
      )}
    </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`w-full rounded-2xl border-2 bg-white/80 px-5 py-4 text-sm font-semibold text-gray-900 shadow-lg outline-none transition-all duration-300 placeholder:font-medium placeholder:text-gray-400 ${error ? 'border-red-300 ring-4 ring-red-100'
    ...
    <div>
      ...
      <div className="relative">
        <select
          ...
          onFocus={(e) => { setFocused(true); props.onFocus?.(e); }}
          onBlur={(e) => { setFocused(false); props.onBlur?.(e); }}
          className={`w-full appearance-none rounded-2xl border-2 bg-white/80 px-5 py-4 pr-12 text-sm font-semibold text-gray-900 shadow-lg outline-none transition-all duration-300 ${error ? 'border-red-300 ring-4 ring-red-100'
              : focused ? 'border-green-400 bg-white ring-4 ring-green-100'
                : 'border-gray-200/60 hover:border-green-300'
            }`}
        >
...
export default function UserFormScreen({
  ...
  // ✅ Derived helpers
  const isInviteFlow = Boolean(inviteData);

  const inviteAssignedFranchiseName =
    inviteData?.franchiseName ||
    inviteData?.operatorName ||
    inviteData?.franchiseOperatorName ||
    'Assigned via invite';

  // ✅ Backend now sends franchiseId in invite verify response
  const inviteFranchiseId = inviteData?.franchiseId || '';
  ...
  // Fetch franchises (only for direct signup)
  ...
  // ✅ VALIDATE — invite uses inviteFranchiseId, direct uses form.franchiseId
  const validateStep = (s) => {
    ...
    if (s === 3) {
      ...
      if (isInviteFlow) {
        if (!inviteFranchiseId) e.franchiseId = 'Franchise not found in invitation';
      } else {
        if (!form.franchiseId) e.franchiseId = 'Select a franchise';
      }
    }
    if (s === 4) {
  ...
  // ✅ SUBMIT — always sends franchiseId (from invite or from dropdown)
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Final franchiseId: invite → inviteFranchiseId, direct → form.franchiseId
      const finalFranchiseId = isInviteFlow
        ? Number(inviteFranchiseId)
        : Number(form.franchiseId);
      ...
      const payload = {
        ...
        ...(form.businessSector === 'OTHER' && {
          otherBusinessSector: form.otherBusinessSector.trim(),
        }),
        country: capitalizeFirstLetter(form.country),
        ...(form.state && { state: capitalizeFirstLetter(form.state) }),
        ...(form.city && { city: capitalizeFirstLetter(form.city) }),
      ...
      console.log('📨 finalFranchiseId:', finalFranchiseId);
      ...
      onComplete?.({
        ...
        data: {
          ...data,
          businessPartnerApplicationStatus: isBusinessPartner
            ? (data?.businessPartnerApplicationStatus || 'PENDING')
            : null,
          franchiseName:
            data?.franchiseName ||
            inviteAssignedFranchiseName ||
            getSelectedFranchise()?.name ||
            '',
        },
      });
  ...
  return (
    <>
      ...
      <div
        className="form-scroll relative"
          ...
          >
            <div className="relative mb-4 flex items-center justify-between">
              ...
              {[
                ...
                >
                  <div className={`relative flex h-14 w-14 items-center justify-center rounded-2xl font-bold shadow-xl transition-all duration-500 ${s.num < step ? 'bg-gradient-to-br from-green-500 to-emerald-600 text-white'
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-black text-green-800">
                    Invited to {inviteAssignedFranchiseName}
                  ...
                  <div className="relative rounded-3xl border-2 border-gray-200/60 bg-white/80 p-8 shadow-2xl backdrop-blur-xl sm:p-10">
                    ...
                    <div className="space-y-6">
                      <PhoneInputWithCode
                        ...
                        countryCode={form.alternatePhoneCountryCode}
                          ...
                          {CONTACT_METHOD_OPTIONS.map((m) => {
                            ...
                            return (
                              <motion.button key={m.value} type="button"
                                ...
                                className={`relative rounded-2xl border-2 p-5 text-center transition-all duration-300 ${active ? 'border-green-400 bg-gradient-to-br from-green-50 to-emerald-50 shadow-xl'
                          ...
                          {LANGUAGE_OPTIONS.map((lang) => {
                            ...
                            return (
                              <motion.button key={lang.value} type="button"
                                ...
                                className={`relative rounded-2xl border-2 p-4 text-center transition-all duration-300 ${active ? 'border-green-400 bg-gradient-to-br from-green-50 to-emerald-50 shadow-xl'
                                    : 'border-gray-200/60 bg-white/80 hover:border-green-300'
                                  }`}
                              >
                ...
                >
                  <div className="relative rounded-3xl border-2 border-gray-200/60 bg-white/80 p-8 shadow-2xl backdrop-blur-xl sm:p-10">
                    <div className="relative mb-8 flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-500">
                          {isInviteFlow ? 'Confirm your location details' : 'Select country, then choose your franchise'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      {/* ✅ FRANCHISE SECTION */}
                      {isInviteFlow ? (
                        /* ── Invite flow: show assigned franchise, NO dropdown ── */
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                          className="rounded-2xl border-2 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 p-5"
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 text-2xl shadow-md">
                              ⭐
                            </div>
                            <div className="flex-1">
                              <p className="text-sm font-black text-green-800">
                              ...
                              <p className="text-xs font-medium text-green-600">
                                Assigned via invitation — no selection needed
                              </p>
                              {inviteFranchiseId && (
                                <p className="text-[10px] font-semibold text-green-500 mt-0.5">
                                  Franchise ID: {inviteFranchiseId}
                                </p>
                              )}
                            </div>
                        ...
                        /* ── Direct signup: show franchise dropdown ── */
                          ...
                          <SearchableDropdown
                            ...
                            renderItem={(franchise, selected, onClick) => (
                              <button type="button" onClick={onClick}
                                className={`flex w-full items-start gap-4 px-5 py-4 text-left transition-all ${selected ? 'bg-gradient-to-r from-green-50 to-emerald-50' : 'hover:bg-gray-50'
                                  }`}
                              >
                                <div className={`mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-lg shadow-md ${franchise.type === 'GENERAL'
                                ...
                                <div className="min-w-0 flex-1">
                                  <p className={`mb-1.5 text-sm font-bold ${selected ? 'text-green-700' : 'text-gray-800'}`}>
                                    {franchise.name}
                                  </p>
                                  <div className="flex flex-wrap items-center gap-2">
                                    <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${franchise.type === 'GENERAL' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                      }`}>{franchise.type}</span>
                                    {franchise.sectorName && (
                                      <span className="inline-flex rounded-full bg-purple-100 px-2.5 py-1 text-[10px] font-bold text-purple-700">
                                        {franchise.sectorName}
                                      </span>
                                    )}
                      ...
                      {ROLE_OPTIONS.map((role) => {
                        ...
                        return (
                          <motion.button key={role.value} type="button"
                            ...
                            className={`group relative flex w-full items-center gap-5 rounded-2xl border-2 p-6 text-left transition-all duration-300 ${active ? 'border-green-400 bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 shadow-2xl shadow-green-500/20'
                                : 'border-gray-200/60 bg-white/80 hover:border-green-300 hover:shadow-xl'
                              }`}
                          >
                            <div className={`relative z-10 flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl text-4xl shadow-xl transition-all duration-300 ${active ? `bg-gradient-to-br ${role.color}` : 'bg-gray-100'
                              }`}>
                              {role.icon}
                            </div>
                            ...
                            <div className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-300 ${active ? 'border-green-500 bg-green-500 shadow-lg' : 'border-gray-300 bg-white'
                              }`}>
                              {active && (
                                <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 500 }}>
                        ...
                        {EVENT_INTEREST_OPTIONS.map((interest) => {
                          ...
                          return (
                            <motion.button key={interest.value} type="button"
                              ...
                              className={`relative flex flex-col items-center gap-2 rounded-2xl border-2 p-4 text-center transition-all duration-300 ${active ? 'border-indigo-400 bg-gradient-to-br from-indigo-50 to-purple-50 shadow-xl shadow-indigo-500/20'
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        ...
                        <SummaryCard label="Phone"
                          value={form.alternatePhoneNumber
                            ? `${form.alternatePhoneCountryCode} ${form.alternatePhoneNumber}` : ''}
                          icon="📱"
                        />
                        ...
                        <SummaryCard
                          label="Franchise"
                          value={isInviteFlow ? inviteAssignedFranchiseName : (getSelectedFranchise()?.name || '-')}
                          icon="⭐"
                        />
                      ...
                      {form.eventInterests.length > 0 && (
                        <div className="mt-3 rounded-xl border-2 border-gray-200/60 bg-white/80 p-3">
                          <div className="mb-2 flex items-center gap-2">
